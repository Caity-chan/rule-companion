...
module.exports = {
  ...
  executableBy: "Administrator",
    ...
    if (args[0] === undefined) {
      ...
      message.channel.send("Successfully set welcome message channel to " + `#${message.channel.name}` + "!")
    } else {
      const channel = message.mentions.channels.first().id;
      wc.set(`welc.${message.guild.id}`, `${channel}`);
      message.channel.send("Successfully set welcome message channel to " + `#${message.mentions.channels.first().name}` + "!")
    }
